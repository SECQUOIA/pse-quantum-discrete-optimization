...
A current 524288-read metadata rerun of those 3 selected VQE seeds is committed
separately for provenance. It produced 4 global-optimum reads total
(`74018: 0`, `74007: 1`, `74001: 3`), so it records current simulator metadata
and optimized parameter vectors without replacing the historical cached result
above.
...
- the best direct full-QUBO QAOA final-sampling row has 5 top-50 repaired-flow
  ...
  512 reads,
- the optimized direct full-QUBO p=2 high-read QAOA run has 136 top-50
  repaired-flow hits, 27 top-10 hits, 4 global hits, and best repaired
  objective `11.7095` in 32768 reads,
...
- `ds_mfg_direct_full_qubo_qaoa_highread/direct_full_qubo_qaoa_optimized_parameters.json`
  freezes the optimized p=2 beta-then-gamma QAOA parameters for the direct
  full-QUBO hardware handoff script,
...
Reduced-surrogate VQE reruns write `vqe_reduced_seed*_metadata.json` files
containing the QiskitOpt `SampleSet` metadata, including initial parameters,
optimized parameters when exposed, optimizer result fields, backend settings,
and seed metadata. The committed seed-74018 metadata rerun uses 128 optimizer
reads, 32768 final reads, and 25 function evaluations. The selected-seed
metadata rerun uses the same optimizer settings with 524288 final reads for
seeds 74018, 74007, and 74001. These artifacts record optimized parameter
vectors and current simulator metadata but are not replacements for the
higher-read final VQE follow-up cache.
...
- `ds_mfg_gurobi_provenance/gurobi_pool_metadata.json`: recovered retained
  Gurobi model and pool metadata, with unavailable status, version, `MIPGap`,
  and `PoolGap` fields marked explicitly.
- `ds_mfg_gurobi_provenance/ip_exact_enumeration_summary.json` and
...
- `ds_mfg_direct_full_qubo_audit/`: direct original-QUBO audit summaries,
  resource metadata, and the fixed p=2 QAOA hardware parameter artifact.
- `ds_mfg_direct_full_qubo_qaoa_highread/`: optimized direct full-QUBO p=2 QAOA
  high-read sample, distribution, and hardware handoff parameter artifact.
- `ds_mfg_direct_full_qubo_hardware_pilot/`: real `ibm_fez` direct full-QUBO
  p=2 hardware pilot using the optimized high-read parameter artifact.
- `ds_mfg_vqe_reduced_flow_objective_seed74018_metadata/`: selected
  reduced-surrogate VQE seed-74018 rerun with optimized `EfficientSU2`
  parameter metadata.
- `ds_mfg_vqe_reduced_flow_objective_selected_metadata/`: final-budget
  selected-seed VQE metadata rerun for seeds 74018, 74007, and 74001.
...
- `scripts/run_direct_full_qubo_hardware_pilot.jl`: dry-run-safe IBM Runtime
  handoff for the direct 36-variable full-QUBO p=2 QAOA circuit using the
  optimized high-read parameter artifact when present; set
  `DSMFG_RUN_DIRECT_FULL_QUBO_HARDWARE=true` to submit real hardware jobs.
...
- `scripts/revisit_reduced_vqe.jl`: reduced-surrogate VQE rerun helper; set `DSMFG_REVISIT_OUTPUT_DIR`, `DSMFG_REVISIT_SEEDS`, and `DSMFG_REVISIT_FINAL_READS` to store new sweeps separately. It writes per-seed VQE metadata JSON artifacts next to the summary and hit CSVs.
