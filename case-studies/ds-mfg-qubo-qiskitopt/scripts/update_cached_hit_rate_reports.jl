...
const SUMMARY_TARGETS = [
    (
        ...
        feasible_relpath = "ds_mfg_qaoa_juliqaoa_transfer/qaoa_juliqaoa_transfer_hits.csv",
        feasible_count_column = "reads",
        feasible_key_columns = ("algorithm", "angle_target", "p", "seed"),
    ),
    (
        relpath = "ds_mfg_qaoa_juliqaoa_transfer_highread/qaoa_juliqaoa_transfer_summary.csv",
        time_column = "solve_time_sec",
        feasible_relpath = "ds_mfg_qaoa_juliqaoa_transfer_highread/qaoa_juliqaoa_transfer_hits.csv",
        feasible_count_column = "reads",
        feasible_key_columns = ("algorithm", "angle_target", "p", "seed"),
    ),
    (
        ...
        time_column = "solve_time_sec",
        feasible_relpath = "ds_mfg_vqe_reduced_flow_objective_v3/vqe_reduced_top50_sampling_hits.csv",
        feasible_count_column = "reads",
        feasible_key_columns = ("algorithm", "seed"),
    ),
    (
        relpath = "ds_mfg_vqe_reduced_flow_objective_v4/vqe_reduced_top50_sampling_summary.csv",
        time_column = "solve_time_sec",
        feasible_relpath = "ds_mfg_vqe_reduced_flow_objective_v4/vqe_reduced_top50_sampling_hits.csv",
        feasible_count_column = "reads",
        feasible_key_columns = ("algorithm", "seed"),
    ),
    (
        ...
        time_column = "solve_time_sec",
        feasible_relpath = "ds_mfg_vqe_reduced_flow_objective_final/vqe_reduced_top50_sampling_hits.csv",
        feasible_count_column = "reads",
        feasible_key_columns = ("algorithm", "seed"),
    ),
    (
        ...
        feasible_relpath = "ds_mfg_classical_baselines/classical_baseline_retained_flows.csv",
        feasible_count_column = "visits",
        feasible_key_columns = ("algorithm", "seed", "sample_budget"),
    ),
]
...
function remove_stat_columns(header::Vector{String}, row::Vector{String})
    keep_indices = [index for (index, name) in enumerate(header) if !(name in GENERATED_COLUMNS)]
        ...
        for (event, hit_column) in HIT_RATE_EVENTS
            ...
            push!(
                ...
                [
                    ...
                    format_stat_value(stats.tts99_sec),
                ],
            )
        end
    end
