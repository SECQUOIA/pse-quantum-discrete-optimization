...
A complete quantum workflow also includes reformulation, exhaustive enumeration and repair, surrogate fitting, offline angle optimization, circuit compilation, and, on hardware, queuing and result retrieval.
Timings for these stages were not retained for the original runs, and the 73.5~s hardware record does not separate compilation, queuing, and execution, so an end-to-end runtime cannot be reconstructed and the sampling-only TTS values understate the quantum cost.
