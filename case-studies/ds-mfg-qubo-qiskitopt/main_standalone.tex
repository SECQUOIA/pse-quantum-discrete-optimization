...
Refitting the surrogate to uniform random subsets of 1024 to 65536 flows (three seeds per size) changes neither its $R^2$ (0.9988--0.9991) nor this failure (minimizers repair to 14.55--18.18), so the failure is a property of the quadratic form rather than of the training set.
...
We tested how the results change with penalty weights, QAOA depth, optimizer budget, and random seeds, and we compared additional classical and annealing baselines.
...
Two further tests concern the offline angle search of energy-targeted QAOA.
The first varies circuit depth: repeating the search for $p=1$ to $5$ with three random seeds, the state-vector optimum probability rises from $1.42\times10^{-4}$ at $p=1$ to $(2.63$--$2.81)\times10^{-3}$ at $p=5$, consistent with the sampled rate in Table~\ref{tab:ds-mfg-primary-outcomes}.
The second varies the optimizer budget at fixed $p=5$: running the basin-hopping search with five steps (random angle perturbations, each followed by a BFGS local search) instead of a single step leaves the optimum probability at $2.63\times10^{-3}$.
Depth, not optimizer budget, controls how strongly the samples concentrate.
...
Finally, we compared simulated annealing~\cite{Kirkpatrick1983}, uniform sampling, and hill climbing with energy-targeted QAOA at a common budget of 262144 evaluations or reads, using five seeds for each classical method.
Annealing found the optimum 9--25 times per run, hill climbing 17--27 times, and uniform sampling at most once, whereas the $p=5$ QAOA circuit returns it about 690 times at its state-vector probability (664 observed in Table~\ref{tab:ds-mfg-primary-outcomes}).
Each classical run took 0.6--2.3~s, against 53.9~s of QAOA sampling preceded by roughly 100~s of offline angle search, so QAOA gives the most concentrated distribution but the slowest route to the optimum.
Quantum annealing lies outside the gate-based scope of this paper; our earlier study of the same instance~\cite{park2026arxiv} reports that simulated annealing and the D-Wave Advantage2 annealer each recover all 36 feasible designs in a single run of 1000 reads, with 99\% time-to-optimum of 197.8 and 199.6~s, while Advantage~4.1 recovers 28 designs (311.9~s of quantum processing time).
Those metrics score raw annealer samples on different hardware and read budgets, so they complement Table~\ref{tab:ds-mfg-primary-outcomes} rather than extend it.
...
Table~\ref{tab:ds-mfg-primary-outcomes} is not an end-to-end cost comparison.
Its call times cover the sampling stage only, whereas a complete quantum workflow also includes reformulation, exhaustive enumeration and repair, surrogate fitting, offline angle optimization, circuit compilation, and, on hardware, queuing and result retrieval.
Those stage times were not recorded for the original runs, and the 73.5~s hardware record does not separate compilation, queuing, and execution, so the quantum cost cannot be totaled.
The sampling-only TTS values therefore flatter the quantum rows, and a full accounting could only widen the classical margin.
