...
report empirical hit rates for the top-50, top-10, global-optimum, and
Gurobi-pool-feasible events. The Gurobi-pool-feasible event counts sampled flows
...
Representative global-optimum and Gurobi-pool-feasible-hit rows are:
...
carry the corresponding top-50, top-10, global, and
...
scored 6 top-50 repaired-flow hits, 1 top-10 hit, 0 global-optimum hits, and 4
Gurobi-pool-feasible hits. The best repaired flow was rank 2 with objective
...
| FakeFez/Aer model-based noisy simulation | overnight script available; no cached result yet | pending | pending | pending |
| IBM `ibm_fez` hardware pilot | 36864 reads | 6 (0.000162760) | 1 (0.0000271267) | 0 (0) |

Relative to the ideal Aer rates, the hardware pilot retained about 0.068% of
the top-50 hit rate and about 0.050% of the top-10 hit rate, with no observed
global-optimum reads. The model-based noisy simulation path is provided by
`scripts/run_noisy_qaoa_fake_backend.jl`. It uses
`qiskit_ibm_runtime.fake_provider.FakeFez` and
`qiskit_aer.AerSimulator.from_backend` with hardware-comparable default shots,
but it is expected to run overnight; local timing probes took 17.5 seconds for
64 shots and 116.6 seconds for 512 shots on the depth-1557, 1141-CZ transpiled
circuit. Any FakeFez/Aer result should be labeled as model-based simulation,
not as a calibrated prediction of the hardware run.
