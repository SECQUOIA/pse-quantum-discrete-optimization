...
export and QUBO archive. In that export the model is named \texttt{capex}; it
has 19 binary flow variables, 20 constraints, 58 nonzeros, and a minimization
objective. The retained Gurobi export records solution-pool search with
\texttt{PoolSearchMode=2}, \texttt{PoolSolutions=36}, 36 retained solutions,
and a solve time of 0.0733 s, with the best retained objective equal to
$11.7095$. The export does not retain the Gurobi version, final status code,
\texttt{MIPGap}, or \texttt{PoolGap}; those fields are now explicitly tracked
as unavailable in
\texttt{ds\_mfg\_gurobi\_provenance/gurobi\_pool\_metadata.json}. We use this
pool as classical ground truth over the original 19-variable decision space,
while avoiding an exhaustive-pool claim not supported by the retained export.
...
per seed. A selected seed-74018 reduced-surrogate rerun is committed under
\texttt{ds\_mfg\_vqe\_reduced\_flow\_objective\_seed74018\_metadata/}; it
records the QiskitOpt \texttt{SampleSet} metadata, including initial VQE
...
The selected seed-74018 metadata rerun stores the optimized
\texttt{EfficientSU2} parameter vector for a 32768-read reduced-surrogate VQE
run with 128 optimizer reads and 25 function evaluations. That rerun produced
five Gurobi-pool top-50 hits and no global hits, so it is used as provenance for
the fixed-parameter vector rather than as a replacement for the higher-read
final VQE follow-up.
...
The VQE hardware candidate remains a follow-up item. The committed cache
identifies seed 74018 as the best final VQE follow-up, and the selected
seed-74018 metadata rerun now persists an optimized \texttt{EfficientSU2}
parameter vector for a clean fixed-parameter hardware-sampling handoff. The
32768-read metadata rerun is not used to replace the higher-read final VQE hit
counts because it produced no global hits at that smaller sampling budget.
...
\texttt{Fw\_ DS mfg case qubo information.zip}. Exact auxiliary repair and the
reduced flow objective are cached under
...
optimized-parameter metadata rerun is stored in
\texttt{ds\_mfg\_vqe\_reduced\_flow\_objective\_seed74018\_metadata/}.
