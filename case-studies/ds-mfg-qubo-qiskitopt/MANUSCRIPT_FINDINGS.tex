...
objective. The Gurobi solve was run with solution-pool search enabled and
returned 36 pool solutions. We use this pool as classical ground truth over the
original 19-variable decision space.
...
The VQE hardware candidate remains a follow-up item. The cache identifies seed
74018 as the best final VQE follow-up, but it does not yet persist the optimized
\texttt{EfficientSU2} parameter vector needed for a clean fixed-parameter
hardware sampling experiment.
...
\texttt{ds\_mfg\_direct\_full\_qubo\_qaoa\_highread/}. Classical context is
