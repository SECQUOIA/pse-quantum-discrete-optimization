...
\texttt{QiskitOpt.jl v0.5.0}, \texttt{QUBODrivers v0.6.1},
...
\texttt{QUBODrivers.RandomSeed()} so \texttt{QiskitOpt.jl v0.5.0} derives local
...
\texttt{qiskit\_aer.AerSimulator.from\_backend}; dry-run safe by default;
overnight defaults are 4096 shots, 3 repeats, transpile seeds 92001--92003. &
...
\centering
\scriptsize
\caption{Ideal simulator, model-based noisy-simulation path, and hardware
comparison for the fixed top-10-targeted $p=5$ QAOA circuit. Rates in
parentheses are empirical hit rates.}
\label{tab:ds-mfg-simulator-hardware-comparison}
\begin{tabular}{p{0.24\linewidth}p{0.22\linewidth}p{0.16\linewidth}p{0.16\linewidth}p{0.14\linewidth}}
\hline
...
\hline
\texttt{FakeFez}/Aer model-based noisy simulation & Overnight script
available; no cached result yet & \emph{pending} & \emph{pending} &
\emph{pending} \\
\hline
...
scoring path can be executed on IBM hardware. The observed hardware
distribution was much less concentrated than the corresponding local Aer
distribution: the local top-10-targeted $p=5$ Aer run sampled the global
optimum 1007 times in 262144 reads, whereas the hardware pilot observed no
global-optimum reads in 36864 shots and only one top-10 repaired-flow read.
Relative to the ideal Aer rates, the hardware pilot retained approximately
0.068\% of the top-50 hit rate and 0.050\% of the top-10 hit rate, with zero
observed global-optimum reads. The cached tables are
...
The pinned Qiskit stack exposes
\texttt{qiskit\_ibm\_runtime.fake\_provider.FakeFez} and
\texttt{qiskit\_aer.AerSimulator.from\_backend}, so a model-based noisy
simulation can be run with
\texttt{scripts/run\_noisy\_qaoa\_fake\_backend.jl}. It is not cached here
because the local timing probe was slow: the \texttt{FakeFez}-transpiled
circuit had depth 1557 and 1141 \texttt{cz} gates, and took 17.5 seconds for
64 shots and 116.6 seconds for 512 shots. The default script budget therefore
targets an overnight run with the same 4096-shot, three-repeat, three-seed
budget used by the hardware pilot. Any such result should be labeled as a
model-based simulation and not as a calibrated prediction of the hardware run.
